...
async function bestSeller() {
  try {
    ...
    const container = document.getElementById("best-seller");

    const bestSellers = products.filter(p => p.sold > 300);
        ...
        <div class="bg-[#121212] border border-[#2a2a2a] rounded-xl shadow-md hover:shadow-2xl hover:border-[#D4AF37] transition duration-300 overflow-hidden group flex flex-col h-full">
          ...
          <div class="relative overflow-hidden shrink-0">
            <img 
              ...
              class="w-full h-56 object-cover transition duration-500 group-hover:scale-110"
            />
            <div class="absolute top-4 left-4 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-tighter">
              Best Seller
            </div>
          </div>
          ...
          <div class="p-6 flex flex-col grow">
            ...
            <div class="flex items-center gap-1 mb-4 text-xs text-gray-400">
              <span><span class="text-white font-semibold">${p.sold.toLocaleString('id-ID')}</span> terjual</span>
            </div>
